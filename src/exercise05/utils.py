...
def recursive_update(obj: Union[dict, object], updates: dict):
    ...
    if isinstance(obj, dict):
        for k, v in updates.items():
            if (
                k in obj
                and (isinstance(obj[k], dict) or dataclasses.is_dataclass(obj[k]))
...
def get_equilibrium(
    ...
    try:
        solver = cs.nlpsol(
            "eq", "ipopt", nlp, {"print_time": 0, "ipopt.print_level": 0, "ipopt.sb": "yes"}
        )
        z0 = np.zeros(nx + nu)
        sol = solver(x0=z0)
...
def select_inducing_points(
    ...
    def fps_select(
        ...
        N_ = Xs_.shape[0]
        selected = (
            preselected.clone().tolist() if preselected is not None and len(preselected) > 0 else []
        )
