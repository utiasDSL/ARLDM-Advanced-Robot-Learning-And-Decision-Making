...
class BaseGaussianProcess:
    def __init__(
        ...
        assert isinstance(X_train, (type(None), np.ndarray)) and isinstance(
            y_train, (type(None), np.ndarray)
        ), "X_train and y_train must be numpy arrays or None."
    ...
    def matern_kernel(self, X1, X2):
        ...
        if nu == 0.5:
            ...
            sqrt3 = np.sqrt(3)
            matern_kernel = (1 + sqrt3 * dists / length_scale) * np.exp(
                -sqrt3 * dists / length_scale
            )
        elif nu == 2.5:
            sqrt5 = np.sqrt(5)
            matern_kernel = (
                1 + sqrt5 * dists / length_scale + 5 * dists**2 / (3 * length_scale**2)
            ) * np.exp(-sqrt5 * dists / length_scale)
        else:
...
class IndependentMultitaskSVGPModel(gpy.models.ApproximateGP):
    def __init__(self, X_train, num_tasks, num_inducing_points):
        ...
        inducing_points = torch.stack(
            [
                select_inducing_points(X_train, num_inducing_points, mode="kmeans", seed=i)
...
class SVGPTrainer:
    ...
    def infer(
        self, X_test: Union[np.ndarray, torch.Tensor], return_std=True, return_ci=False
