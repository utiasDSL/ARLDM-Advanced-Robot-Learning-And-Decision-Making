...
def load_state_transitions(data_path: str = None):
    """Loads state transition data from a specified file."""
    if data_path is None:
        cwd = os.getcwd()
        data_path = os.path.join(cwd, "outputs", "state_transitions.pkl")

    assert os.path.exists(data_path), (
        f"Data file {data_path} does not exist. "
        "Please run collect_state_transitions or pull from the repo to generate the data."
    )
...
def collect_state_transitions(
    ...
    Args:
        ...
        noise_scale (float): Standard deviation of the Gaussian noise to be added as a fraction of the signal.
        n_episodes (int): Number of episodes to run for data collection.
    ...
    if load_if_exists:
        return load_state_transitions(data_path)

    # Extra imports (not available for the students)
    import gymnasium
    ...
    assert os.path.exists(ppo_checkpoint_path), (
        f"Checkpoint file {ppo_checkpoint_path} does not exist. Please provide a valid path to the PPO checkpoint."
    )
    ...
    env = gymnasium.make_vec("DroneFigureEightTrajectory-v0", num_envs=n_envs, device=device)
    ...
    # Load policy
    checkpoint = torch.load(ppo_checkpoint_path, weights_only=True, map_location=torch.device(device))
    ...
    if noisy:
        ...
        for key in ["states", "next_states", "actions"]:
            data = recorded_data[key]
            noise = np.random.normal(0, noise_scale, data.shape)
            recorded_data[key] = data * (1 + noise)
    # Save data
    os.makedirs(save_dir, exist_ok=True)
    with open(data_path, "wb") as f:
        pickle.dump(recorded_data, f)
    print(f"Saved state transitions to {data_path}.")
    return recorded_data
