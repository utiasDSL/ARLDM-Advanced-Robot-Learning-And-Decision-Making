...
def make_envs(
    env_name: str, n_envs: int, n_eval_envs: int, train_device: str, **kwargs: dict[str, Any]
    ...
    train_envs = gymnasium.make_vec(env_name, num_envs=n_envs, freq=50, device=env_device, **kwargs)
    eval_envs = gymnasium.make_vec(
        env_name, num_envs=n_eval_envs, freq=50, device=env_device, **kwargs
    )
...
def save_model(
    ...
    """
    save_dict = {"model_state_dict": None, "optim_state_dict": None}
    ########################################################################
    # TODO:
...
class PPOTrainer:
    def __init__(
        ...
        **kwargs: dict[str, Any],
        ...
        if wandb_log:
            wandb.init(
                project=f"ARLDM-Exercise-PPO-{self.config.env_name}", config=None, reinit=True
            )
