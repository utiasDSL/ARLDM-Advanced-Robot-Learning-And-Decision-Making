...
def init_gp(kernel="se", kernel_params={"length_scale": 1.0, "nu": 1.5}, noise=1e-4):
    return MultiOutputGaussianProcessStudent(
        kernel=kernel, kernel_params=kernel_params, noise=noise
    )
...
class TestKernels(BaseTest):
    def kerneltest(self, kernel: str):
        ...
        self.assertEqual(
            K.shape, (self.X1.shape[0], self.X2.shape[0]), f"{kernel} kernel shape is not correct."
        )
    ...
    def test_init_SVGP(self):
        ...
        try:
            gp = SVGPStudent(
                ...
                num_inducing_points=self.X_train.shape[0] // 2,
            )
        except Exception as e:
            self.fail(f"SVGP initialization failed with exception: {e}")

        self.assertTrue(
            gp.var_distribution_type == gpy.variational.CholeskyVariationalDistribution,
            "Variational distribution is not of the correct type.",
        )
        self.assertTrue(
            gp.var_strategy_type == gpy.variational.IndependentMultitaskVariationalStrategy,
            "Variational strategy is not of the correct type.",
        )
...
class TestCustomDatasetInit(unittest.TestCase):
    ...
    def test_iter_shuffle(self):
        ...
        dataset = CustomDataset(self.X, self.y, batch_size=self.batch_size, shuffle=True)
    ...
    def test_iter_no_shuffle(self):
        ...
        dataset = CustomDataset(self.X, self.y, batch_size=self.batch_size, shuffle=False)
        batches = list(iter(dataset))
        self.assertEqual(len(batches), len(dataset))
        for X_batch, y_batch in batches:
            self.assertEqual(X_batch.shape[0], y_batch.shape[0])
...
class TestNeuralNetworkInit(unittest.TestCase):
    ...
    def test_init_NN(self):
        ...
        try:
            model = NeuralNetwork(init_from_checkpoint=True, checkpoint_path=self.checkpoint_path)
        except Exception as e:
            self.fail(f"Initialization of the Neural Network class failed with exception: {e}")
        self.assertIsInstance(model, NeuralNetwork, "Model is not an instance of NeuralNetwork")
...
class TestTrainEpoch(unittest.TestCase):
    def setUp(self):
        set_seed()
        checkpoint_path = os.path.join(import_prefix, "outputs", "nn_checkpoint_ex04.ckpt")
        self.model = NeuralNetwork(init_from_checkpoint=True, checkpoint_path=checkpoint_path)
        # Learn a simple linear relationship
        coefficients = torch.randn(
            self.model.hyperparameters["input_dim"], self.model.hyperparameters["output_dim"]
        )
        ...
        self.train_loader = CustomDataset(self.x_train, self.y_train, batch_size=8, shuffle=True)
    ...
    def test_optimizer_scheduler_criterion(self):
        ...
        self.assertTrue(
            self.trainer.scheduler is None
            or isinstance(self.trainer.scheduler, torch.optim.lr_scheduler.LRScheduler),
    ...
    def test_train_epoch(self):
        ...
        for initial, updated in zip(initial_params, updated_params):
            self.assertFalse(torch.equal(initial, updated), "Model parameters did not update.")
        ...
        self.assertLess(current_loss, loss, "Loss did not decrease over first 20 epochs")
