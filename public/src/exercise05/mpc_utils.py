...
class OcpRegistry:
    ...
    def add_param(
        ...
        if init is None:
            init = np.zeros(size)
        assert init.shape == (size,), (
            f"Init for param {name} must be shape {(size,)}, got {init.shape}"
        )
        if sym is None:
            sym = cs.MX.sym(f"p__{name}", size, 1)
        self.parameters[name] = ParameterBlock(
            name=name, sym=sym, values=init.copy(), update_fcn=update_fcn
        )
        return sym
    ...
    def update_params(self, solver: AcadosOcpSolver, which: Union[list[str], bool] = True):
        if not self.parameters:
            return
        assert isinstance(which, (list, str, bool)), (
            "which must be a list of parameter names or a bool"
        )
        ...
        for blk in self.parameters.values():
            ...
            if vals.ndim == 1 or vals.shape[1] == 1:
                ...
                assert vals.shape[1] == self.horizon + 1, (
                    f"Time-varying param {blk.name} must have second dim T+1"
                )
                tiled = vals.T
            else:
    ...
    def build_constraints(
        self, ocp: AcadosOcp, tol: float = 1e-4, big_neg: float = -1e8, big_pos: float = 1e8
    ):
        nh_0 = nh = nh_e = 0
        con0, con, cone = [], [], []
        idxsh, idxsh_e = [], []
        ...
        ocp.constraints.ush = (
            np.array(max_slack_list) if max_slack_list else np.full(len(idxsh), big_pos)
        )
        ocp.constraints.lsh_e = np.zeros(len(idxsh_e))
        ocp.constraints.ush_e = (
            np.array(max_slack_list_e) if max_slack_list_e else np.full(len(idxsh_e), big_pos)
        )
