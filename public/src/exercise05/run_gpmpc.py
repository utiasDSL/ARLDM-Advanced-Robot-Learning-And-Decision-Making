...
class Evaluator:
    ...
    def evaluate_epoch(
        self, ctrl: GPMPC, env_seed: int, max_steps: int = 5000, log_steps: int = 50
    ) -> dict:
        ...
        episode_data["ctrl_type"] = (
            ctrl.ctrl_type if hasattr(ctrl, "ctrl_type") else ctrl.__class__.__name__
        )
    ...
    def run(
        self, n_epochs: int = 3, extra_train_runs: bool = True, print_stats: bool = True
    ) -> dict:
        ...
        train_seeds = rng.choice(np.iinfo(np.int32).max, size=int(1e6), replace=False)[
            : n_epochs + 1
        ]
